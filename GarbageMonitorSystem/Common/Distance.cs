...
namespace GarbageMonitorSystem.Common
{
    ...
    {
        private IRoute routemethod;
        public IRoute Routemethod { get => routemethod; set => routemethod = value; }
        public Distance(IRoute routemethod)
        {
        ...
        public static double[,] GetDistanceMatrixSQL(string ids,List<GarbageDistance> garbages)
        {
            ...
            for (int i = 0; i < garbages.Count; i++)
            {
                ...
                array2D[from_index, to_index] = garbages[i].TimeEstimates;
            }
            return array2D;
        }
